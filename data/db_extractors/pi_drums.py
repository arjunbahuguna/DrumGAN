...
def get_standard_format(path: str, dbname='pi-drums'):
    """
    ...
    Args:
        ...
        dbname (str, optional): The name of the dataset. Defaults to 'pi-drums'.
    ...
    ToDo: Writes:
        ...
        dbname.json: Contains path to all filename.json files and other information. See 'description' dict. todo ==> image example of pi-drums.json
    """
    ...
    root_dir = mkdir_in_path(path, f'pi_drums_standardized')
                ...
                for att in keys:
                    if att == 'audio-commons':
                        ...
                        for ac_att in audio_commons_keys:
                            ...
                            # Capture max value for pi-drums.json, the global dataset json
                            if acval > attributes[att]['max'][ac_att]:
                                attributes[att]['max'][ac_att] = acval

                            # Set min value for pi-drums.json, the global dataset json
                            if acval < attributes[att]['min'][ac_att]:
                                attributes[att]['min'][ac_att] = acval

                            # Capture mean value for pi-drums.json, the global dataset json
...
def extract(path: str, criteria: dict={}, download: bool=False):
    ...
    extraction_dict = get_base_db('pi-drums', __VERSION__)
    ...
    for att in out_attributes: #If no criteria passed for attribute, out_attribute is ['instrument', 'audio-commons']
        ...
        # If att not present, define the attribute_dict format. This will be the same as attribute key in pi-drums.json. See format: <attribute_dict.img link> todo
